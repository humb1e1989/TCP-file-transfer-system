...
def step_service(connection_socket, addr):
    ...
    while True:
        ...
        if request_type == TYPE_FILE:
            file_process(username, request_operation, json_data, bin_data, connection_socket) # mistake
            continue
...
def tcp_listener(server_ip, server_port):
    ...
    """
    global logger
    server_socket = socket(AF_INET, SOCK_STREAM) # mistake
    ...
    while True:
        try:
            ...
            th = Thread(target=step_service, args=(connection_socket, addr)) # mistake
            th.daemon = True
            th.start()    # mistake
